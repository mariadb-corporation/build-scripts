...
#        sleep 20
	...
	sudo $qemu_cmd -hda /home/ec2-user/test-machines/test_vm_192.168.122.$i.img -netdev user,id=user.0 -device e1000,netdev=user.0 -boot c  -m 2028 -device e1000,netdev=net0,mac=$mac -netdev tap,id=net0,script=/home/ec2-user/kvm/qemu-ifup --nographic -qmp unix:/tmp/socket$i,server,nowait &
done
